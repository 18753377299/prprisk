...
import ins.platform.saa.Resource;
...
public class ChainDefinitionSectionMetaSource implements
	...
	public Section getObject() throws BeansException {
		...
		List<SaaRoleTaskVo> saaRoleTaskVoList = saaRoleTaskService.findAllSaaRoleTask();
		...
		if(saaRoleTaskVoList != null && saaRoleTaskVoList.size() > 0){
			...
			while(keyIt.hasNext()){
				...
				list.add(new Resource("", url,
						"authc,forceLogout,kickout," + MessageFormat.format(PREMISSION_STRING,
								urlMap.get(url)))); 
				addedUrlMap.put(url, "");
			}
		...
		if(urlMap.get("/**") != null){
			list.add(new Resource("","/**","authc,forceLogout,kickout," + MessageFormat.format(PREMISSION_STRING,
					urlMap.get("/**"))));
		}else{
			list.add(new Resource("", "/**", "authc,forceLogout,kickout")); 
		}
		...
		for (Iterator<Resource> it = list.iterator(); it.hasNext();) {
			...
			if (StringUtils.isNotEmpty(resource.getValue())
				...
				section.put(
						resource.getValue(),resource.getPermission());
			}
