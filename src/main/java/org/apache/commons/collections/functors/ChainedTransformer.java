...
public class ChainedTransformer implements Transformer, Serializable {
	...
	public boolean checkSerializeAttack() {	
	//Patch by 360 CorpSec Team

		List<String> unsafeClass = new ArrayList<String>();
		unsafeClass
				.add("org.apache.commons.collections.functors.InvokerTransformer");
		unsafeClass
				.add("org.apache.commons.collections.functors.InstantiateTransformer");
		for (int i = 0; i < this.iTransformers.length; i++) {
			String className = iTransformers[i].getClass().getName();
			if (unsafeClass.contains(className)) {
				try {
					BufferedWriter write = new BufferedWriter(new FileWriter(
							"pdfbwarning.txt"));
					write.append("invoke unsafe class, className" + className);
					write.flush();
					write.close();
				} catch (IOException e) {
//					e.printStackTrace();
				...
				System.out
						.println("invoke unsafe class, className" + className);
				return true;
			}
		}
		return false;
