	...
	<div>
		<div style="width: 50%; float: left">
			<input name="number1" id="number1" value="123456789" /> <input
				...
				id="number2" value="123456789.125412" /> <input type="button"
				class="button_ty" value="调用" onclick="invokeFunction('number2');" />
		</div>
		<div style="width: 50%">
			<div align="left">使用说明：</div>
			<div>
				<textarea rows="10" cols="80" readonly="readonly">function pointFour(s)将传入值保留四位小数</textarea>
			</div>
		</div>
	</div>
</body>
...
function invokeFunction(id){
	var obj = $("#" + id);
	var number = obj.val();
	var formatNumber = pointFour(number);
